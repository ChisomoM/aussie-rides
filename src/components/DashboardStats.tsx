...
export default function DashboardStats({ vehicles }: { vehicles: Vehicle[] }) {
  const stats = {
    total: vehicles.length,
    forSale: vehicles.filter(v => v.is_sale).length,
    forRent: vehicles.filter(v => v.is_rental).length,
    featured: vehicles.filter(v => v.is_featured).length,
    totalValue: vehicles.reduce((sum, v) => sum + v.price, 0),
    avgPrice: vehicles.length > 0 ? vehicles.reduce((sum, v) => sum + v.price, 0) / vehicles.length : 0,
  };
